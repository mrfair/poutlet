...
module.exports = async function() {
  ...
  fs.writeFileSync(path.resolve('poutlet_config.json'), `{
    "mongodb": {
      "url" : "",
      "db" : ""
    },
  ...
  if(fs.existsSync(path.resolve('poutlet_plugin')) !== true) {
    ...
    fs.writeFileSync(path.resolve('poutlet_plugin/dashboard/method/get/root.js'), `module.exports = function(request, response) { response.html(\`<div><img src="/dashboard/static/logo.png" height="400" /></div><div><i class="fas fa-chart-line"></i> <a href="/dashboard/about">What's about Poutlet.</a></div>\`); return; };`);
    ...
    fs.copyFileSync(__dirname + '/logo.png', path.resolve('poutlet_plugin/dashboard/method/get/static/logo.png'));
  ...
  if(fs.existsSync(path.resolve('poutlet_root/http2_middleware.js')) !== true) {
    fs.writeFileSync(path.resolve('poutlet_root/http2_middleware.js'), 'module.exports = function (request, response) {};');
  }
