...
module.exports = async function() {
  ...
  if(fs.existsSync(path.resolve('poutlet_static')) !== true) {
    ...
    fs.writeFileSync(path.resolve('poutlet_static/global.css'), `body {
      margin: 0;
      padding: 0;
    }`);
  ...
  if(fs.existsSync(path.resolve('poutlet_root')) !== true) {
    ...
    fs.writeFileSync(path.resolve('poutlet_root/get/root.js'), `module.exports = function(request, response) { response.html(\`<i class="fas fa-smile-wink"></i> Hi world. I'm Poutlet. <a href="/dashboard">Go to dasboard</a>\`); return; };`);

    fs.writeFileSync(path.resolve('poutlet_root/get/root.css'), `body { background-color: #F1F2F3; color: #DA0000; }`);

    fs.mkdirSync(path.resolve('poutlet_root/post'));
    
    fs.writeFileSync(path.resolve('poutlet_root/post/root.js'), `module.exports = function(request, response) { response.json({status: 200, responseText: "OK"}); return; };`);
  };
