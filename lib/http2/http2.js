...
var poutlet_css_global =
  fs.existsSync(path.resolve(`poutlet_static/global.css`)) === true
    ? fs.readFileSync(path.resolve(`poutlet_static/global.css`), {
        encoding: "utf8",
        flag: "r",
      })
    : "";
...
module.exports = function (config_http2) {
  try {
          ...
          <html ⚡ lang="en">
            <head>
              ...
              ${_static
                ...
                .join("")}
              <style>${request.poutlet.css}</style>
            </head>
            <body>
              ${body}
              ${_static
    ...
    console.log(
      `HTTP2 server run at https://${config_http2.hostname}:${config_http2.port_dev}`
    );
  } catch (e) {
    console.log(e);
  }
