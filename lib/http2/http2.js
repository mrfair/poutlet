...
module.exports = function (config_http2) {
  ...
  http2.createSecureServer(
    ...
    (request, response) => {
      ...
      if(poutlet_plugin[`${request.poutlet.headers.path_key}_`] !== undefined) {
        _request_poutlet_plugin = poutlet_plugin[`${request.poutlet.headers.path_key}_`];
      } else {
        ...
        for(var i=_hostname_valid.length; i-->0;) {
          if(poutlet_plugin[`${request.poutlet.headers.path_key}_${_hostname_valid[i]}`] !== undefined) {
            _request_poutlet_plugin = poutlet_plugin[`${request.poutlet.headers.path_key}_${_hostname_valid[i]}`];
        ...
        if(_request_poutlet_plugin === null) {
          ...
          response.end(require('../live_reload/client_script.js')(config_http2));
        }
