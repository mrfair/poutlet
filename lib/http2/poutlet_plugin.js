...
module.exports = function (mode) {
  ...
  try {
    ...
    if(mode === 'build') _root_folder_path = path.resolve("poutlet_lib/poutlet_root");


    if (fs.existsSync(_root_folder_path) === true && fs.existsSync(`${_root_folder_path}/method`) === true) {
      _poutlet_plugin["/_*"] = {
        method: {},
        hostname: "*"
      };
      ...
      fs.readdirSync(_root_folder_path + '/method').forEach((method_name) => {
        ...
        if (
          fs.existsSync(`${_root_folder_path}/method/${method_name}`) === true &&
          fs
          ...
          if (
            ...
            _poutlet_plugin["/_*"].method[method_name]["/"].body = require(`${_root_folder_path}/method/${method_name}/root.js`);

            if (
              fs.existsSync(`${_root_folder_path}/method/${method_name}/root.css`)
            ) {
              _poutlet_plugin["/_*"].method[method_name]["/"].css = fs.readFileSync(`${_root_folder_path}/method/${method_name}/root.css`, {
                encoding: "utf8",
                flag: "r",
              });
            }
    ...
    if(mode === 'build') _plugin_folder_path = path.resolve("poutlet_lib/poutlet_plugin");
                  ...
                  fs.readdirSync(path_dir).forEach((file_name) => {
                    ...
                    //file
                      ...
                      if (
                        ...
                        if(!_poutlet_plugin_now_method[
                          `/${plugin_name}${prefix_name}${_file_name_to_path_name}`
                        ].css) _poutlet_plugin_now_method[
                          `/${plugin_name}${prefix_name}${_file_name_to_path_name}`
                        ].css = "";
