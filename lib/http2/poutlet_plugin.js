...
module.exports = function() {
  ...
  if(fs.existsSync(path.resolve('poutlet_root')) === true) {
    ...
    fs.readdirSync(path.resolve('poutlet_root')).forEach((method_name) => {
      ...
      if(fs.lstatSync(path.resolve(`poutlet_root/${method_name}`)).isDirectory()  === true) {

        if(fs.existsSync(path.resolve(`poutlet_root/${method_name}/root.js`))) {
          _poutlet_plugin['/_*'].method[method_name]['/'] = {
              
            headers: {
              "Cache-Control": "no-cache"
            },
            body: require(path.resolve(`poutlet_root/${method_name}/root.js`))
            ...
            fs.readdirSync(`${_plugin_name_folder}/method`).forEach((method_name) => {
              ...
              var read_path_to_plugin = function(path_dir, prefix_name) {
                ...
                fs.readdirSync(path_dir).forEach((file_name) => {
                  ...
                  //Static folder
                  if(file_name === 'static' && fs.lstatSync(`${path_dir}/${file_name}`).isDirectory()  === true) {

                    var _plugin_render_for_static = function(name_of_dir_in_staic) {

                      fs.readdirSync(`${path_dir}/${name_of_dir_in_staic}`).forEach((file_name_in_static) => {
                        if(fs.lstatSync(`${path_dir}/${name_of_dir_in_staic}/${file_name_in_static}`).isDirectory()  === true) {
  
                          _plugin_render_for_static(`${name_of_dir_in_staic}/${file_name_in_static}`);
  
                        } else {
                          zlib.gzip(fs.readFileSync(`${path_dir}/${name_of_dir_in_staic}/${file_name_in_static}`, {encoding:'utf8', flag:'r'}), (err, buffer) => {
      
                            if (err === null) { 
                              _poutlet_plugin_now_method[`/${plugin_name}/${name_of_dir_in_staic}/${file_name_in_static}`] = {
                                headers: {
                  ...
                  //Other folder
                  else if(fs.lstatSync(`${path_dir}/${file_name}`).isDirectory()  === true) {
                    read_path_to_plugin(`${path_dir}/${file_name}`, `${prefix_name}/${file_name}`);
                  }
                  ...
                  //file
                    ...
                    if(file_name !== 'middleware.js' && mime.lookup(file_name) === 'application/javascript') {
                      ...
                      _poutlet_plugin_now_method[`/${plugin_name}${prefix_name}${_file_name_to_path_name}`] = {
                        headers: {
                          "Cache-Control": "no-cache"
                        },
                        body: function(request, response) {
                          ...
                          if (fs.existsSync(`${path_dir}/middleware.js`)) {
                            ...
                            if(typeof _middleware === 'function') _middleware(request, response);
                          }

                          var _plugin_response = require(`${path_dir}/${file_name}`);

                          if(typeof _plugin_response === 'function') _plugin_response(request, response);
                          else {
                            response.writeHead(502);
                            response.end('No function to support response')
                          }
                        }
                      }
              ...
              read_path_to_plugin(`${_plugin_name_folder}/method/${method_name}`, '');
            });
          }
