...
module.exports = function (config_http2) {
  ...
  wsServer.on("request", function (request) {
    ...
    //Global css
    fs.watch(
      path.resolve("poutlet_static/global.css"),
        ...
        if (mime.lookup(filename) === "text/css") {
          connection.sendUTF(
            JSON.stringify({
              ...
              body: fs.readFileSync(path.resolve("poutlet_static/global.css"), {
                encoding: "utf8",
                flag: "r",
              }),
            })
    ...
    //Root css
    fs.watch(
      path.resolve("poutlet_root/get/root.css"),
      function (event, filename) {
        if (mime.lookup(filename) === "text/css") {
          connection.sendUTF(
            JSON.stringify({
              ...
              pathname: "/",
              body: fs.readFileSync(path.resolve("poutlet_root/get/root.css"), {
                encoding: "utf8",
                flag: "r",
              }),
            })
  ...
  setTimeout(function () {
    if (wsServer.connections.length === 0)
      require("open")(
        `https://${config_http2.hostname}:${config_http2.port_dev}`
      );
  }, 500);
