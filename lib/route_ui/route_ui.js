...
module.exports = function (config_http2) {
  try {
    ...
    http2
      .createSecureServer(
        ...
        (request, response) => {
          ...
          for(let i in poutlet_plugin) {
            _row += `<tr><td colspan="100%"><div>Pathname : ${i.split('_'+poutlet_plugin[i].hostname)[0]}</div><div>Hostname: ${poutlet_plugin[i].hostname}</div></td></tr>`;
            ...
            for(var method in poutlet_plugin[i].method) {
              for(var path in poutlet_plugin[i].method[method]) {
                ...
                _row += `<tr valign="top">
                          ...
                          <td id="${method}-text-${encodeURIComponent(path)}" class="pl-text-10"></td>
                        </tr>`;
                ...
                _script += `_test_request("${method}", "${path}", function(response){
                  ...
                  document.getElementById("${method}-text-${encodeURIComponent(path)}").innerText = response.responseText;
                });`
              }
            ...
            <body>
              ...
              <script type="text/javascript">
                ...
                var _test_request = function(method, route, callback) {
                  ...
                  _request.open(method, "https://${config_http2.hostname}:${config_http2.port_dev}" + route, true);
      ...
      console.log(`Monitor server run at https://${config_http2.hostname}:${config_http2.port_ui_api}`);
